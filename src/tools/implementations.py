...
class DocumentLoaderTool(BaseTool):
    ...
    def __init__(self):
        super().__init__(
            ...
            description=
            "Load PDF/HTML/Text from file or URL and return plain text",
            timeout=15,
        )
    ...
    async def execute(self,
        ...
        try:
            ...
            approx_tokens = int(math.ceil(char_length / 4.0))  # rough estimate

            # Optional precise tokenization if tiktoken is available
            tokens_precise = None
            try:
                ...
                if enc is not None:
                    tokens_precise = len(enc.encode(text))
            except Exception as e:
                logger.warning(f"Tiktoken tokenization failed: {e}")
                tokens_precise = None
