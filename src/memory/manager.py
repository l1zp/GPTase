...
from src.memory.storage import MemoryStorage, LocalMemoryStorage
from src.memory.types import Memory, ConversationMemory, TaskMemory, MemoryType
...
class MemoryManager:
    ...
    async def get_task_history(self, agent_id: str = None, limit: int = 20) -> List[TaskMemory]:
    ...
    async def get_next_message(self, agent_id: str, timeout: float = None) -> Optional[ConversationMemory]:
    ...
    async def create_memory_summary(self, agent_id: str = None) -> Dict[str, Any]:
        ...
        if agent_id:
            conversation_history = await self.get_conversation_history(agent_id, limit=100)
