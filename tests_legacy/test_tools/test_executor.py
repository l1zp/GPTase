...
@pytest.mark.asyncio
async def test_tool_results_keep_full_content_when_message_is_truncated(monkeypatch):
    """Trace tool_results must hold the FULL output even when the
    LLM-facing tool message is truncated. Otherwise downstream consumers
    that parse JSON from tool_results.content (e.g.
    runtime._build_coordinator_summary for DelegateTask) will silently
    drop large delegations.
    """
    registry = get_tool_registry()
    original_tools = dict(registry._tools)
    original_permissions = dict(registry._permissions)
    ...
    payload = '{"agent_id": "worker", "status": "success", "content": "' + (
        "X" * 12000) + '"}'
    tool = StaticTool("FullContentTraceTool", payload)
    monkeypatch.setattr(
        registry,
        "_tools",
        {
    ...
    executor = ToolExecutor(model=RecordingModel([]), max_tool_result_chars=800)
    ...
    # The LLM-facing message is truncated as before.
    assert len(result["messages"][-1]["content"]) <= 800
    assert "[TOOL OUTPUT TRUNCATED]" in result["messages"][-1]["content"]

    # But the trace tool_results carry the FULL untruncated payload, so
