...
class TestModel:
    ...
    async def test_shutdown_closes_cached_providers(self):
        """Model.shutdown must close cached providers so AsyncOpenAI httpx
        clients are released. Without this, chat -p runs leak CLOSE_WAIT
        sockets and stall on exit (Slice 1 retro: shutdown hang)."""
        ...
        manager = Model(config)
        fake = FakeProvider()
        manager._provider_cache[("k1", "v1")] = fake

        await manager.shutdown()

        assert fake.closed is True
        assert manager._provider_cache == {}

    async def test_shutdown_tolerates_provider_without_close(self):
        """Providers lacking a close() method must not break shutdown."""

        config = ModelConfig(use_mock=True)
        manager = Model(config)
        manager._provider_cache[("k", "v")] = object()
