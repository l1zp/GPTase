...
你的输入（文本、文档路径、图片）
 ...
 可直接回答，也可请求结构化后续执行
          ...
          v
 [ Auto Orchestrator ]
 可能直接回答，也可能进入 coordinator loop 或创建 harness session
     |                    |
     v                    v
 [Coordinator Loop]   [Harness Session + Plan]
 DelegateTask worker  Draft plan、审核、执行、replan
 回合 + 汇总
```
...
- 如果 runtime 返回 `needs_plan`，orchestrator 才会创建 harness session

**直接回答 vs. 创建 session：**
- direct runtime 回答：不创建 session
- coordinator loop 回答：不创建 session
- plan handoff：创建带 draft plan 的 goal session
...
**是什么：** harness session 是结构化执行层，用在显式 plan 请求或 runtime handoff 之后。

**如何工作：**
- draft plan 可以来自 `plan_id`、`plan_path`、inline plan、普通 plan 生成，或 `runtime_handoff`
- draft plan 中的步骤按顺序或并行组执行
- 使用 `{{step1}}`、`{{step2a.field}}` 模板变量在步骤间传递数据
- session 可等待审核、执行、评估目标，并在需要时自动补充后续 plan
...
2. `Agent.run()` 先走 Claude SDK 或 interactive runtime
3. runtime 可能直接回答、delegation 给 worker，或请求 plan handoff
4. 如果发生 plan handoff，`AgentOrchestrator` 创建 goal session，并挂载 draft plan
5. 每个工作流步骤通过 `TaskDispatcher` 调度到对应 `Agent`
6. 模板变量（`{{step1}}`）从已完成步骤的结果中解析
7. orchestrator 判断目标是否已达成，必要时继续补充 plan
