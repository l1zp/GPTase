...
class HTMLVerifier:
    ...
    def _check_sections(self, html: str, md: str):
        ...
        if len(md_sections
               ) > len(html_sections) + 1:  # +1 for bibliography which is separate
            self.errors.append(
    ...
    def _check_no_placeholders(self, html: str):
        ...
        placeholders = [
            ...
            '{{METRICS_DASHBOARD}}', '{{SOURCE_COUNT}}', 'TODO', 'TBD', 'PLACEHOLDER',
            'FIXME'
        ]
    ...
    def _check_no_emojis(self, html: str):
        ...
        if emojis:
            ...
            self.errors.append(
                f"Found {len(emojis)} emojis in HTML (should be none): {unique_emojis}")
    ...
    def _check_citations(self, html: str, md: str):
        ...
        html_content = html.split(
            'class="bibliography"')[0] if 'class="bibliography"' in html else html
    ...
    def _check_bibliography(self, html: str, md: str):
        ...
        if '## Bibliography' in md:
            if 'class="bibliography"' not in html:
                ...
                self.warnings.append(
                    "Bibliography present but entries not properly formatted")
