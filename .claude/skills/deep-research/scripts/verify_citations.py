...
class CitationVerifier:
        ...
        try:
            ...
            with request.urlopen(req, timeout=10) as response:
                ...
                return True, {
                    ...
                    data.get('title', ''),
                    'year':
                    data.get('issued', {}).get('date-parts', [[None]])[0][0],
                    'authors': [
    ...
    def detect_hallucination_patterns(self, entry: Dict) -> List[str]:
        ...
        if any(word in title.lower()
               for word in generic_words) and len(title.split()) < 5:
        ...
        if entry.get('year'):
            ...
            if year < 2000 and any(word in title.lower()
                                   for word in ['ai', 'llm', 'gpt', 'transformer']):
                issues.append(
        ...
        if entry['doi']:
            ...
            if success:
                ...
                if entry['title'] and metadata.get('title'):
                    similarity = self.check_title_similarity(entry['title'],
                                                             metadata['title'])
    ...
    def verify_all(self):
        ...
        unverified = [
            r for r in results if r['status'] in ['unverified', 'no_doi', 'unknown']
        ]
