...
def split_eval_set(eval_set: list[dict],
                   holdout: float,
...
def run_loop(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        blinded_history = [{
            k: v
            for k, v in h.items() if not k.startswith("test_")
        } for h in history]
        ...
        if verbose:
            print(f"Proposed ({improve_elapsed:.1f}s): {new_description}",
                  file=sys.stderr)
    ...
    if verbose:
        ...
        print(f"Best score: {best_score} (iteration {best['iteration']})",
              file=sys.stderr)
