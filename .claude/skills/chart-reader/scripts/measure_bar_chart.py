...
def detect_bars(img, plot_area, strategy="auto", threshold=200):
    ...
    if strategy in {"auto", "color"}:
        ...
        dark_mask = cv2.morphologyEx(dark_mask, cv2.MORPH_OPEN, np.ones((3, 3),
                                                                        np.uint8))
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract bar heights from a chart image.")
    parser.add_argument("image_path")
    parser.add_argument("--y-ticks",
                        required=True,
                        help="pixel:value pairs, e.g. 290:0,40:100")
    parser.add_argument(
        "--plot-area",
        default=None,
        help="x,y,w,h. Prefer passing this when auto-detection is wrong.")
    ...
    strategy_used, bars, _ = detect_bars(img,
                                         plot_area,
    ...
    if args.debug:
        ...
        for bar in bars:
            overlays.append(
                ("circle", (bar["x_center_pixel"], bar["top_y_pixel"], 5, (0, 0, 255))))
